...
export interface ChildHandoffInput {
  ...
  readonly dependencies: readonly { type: string; id: string; description: string }[];
  readonly risks: readonly { description: string; mitigation?: string }[];
  readonly unresolvedQuestions: readonly string[];
  readonly evidenceRequired: readonly string[];
  readonly repoTargets: { workspaceId: string; packages: string[]; branch?: string };
  readonly source: { providerType: ProviderCapability; providerId: string; sourceRef: string; sourceUrl?: string };
...
export class ChildHandoffService {
  ...
  generate(input: ChildHandoffInput): HandoffProtocolEnvelope {
    return this.handoffFactory.create({
      ...
      partial: {
        acceptanceCriteria: input.acceptanceCriteria.map((desc, i) => ({ id: `ac-${i + 1}`, description: desc })),
        constraints: input.constraints.slice(),
        components: input.components.map((c) => ({ path: c.path, ...(c.description ? { description: c.description } : {}) })),
        ragQueryHints: input.ragQueryHints.map((h) => ({ query: h.query, ...(h.relevanceNote ? { relevanceNote: h.relevanceNote } : {}) })),
        dependencies: input.dependencies.map((d) => ({ type: this.mapDependencyType(d.type), id: d.id, description: d.description })),
        risks: input.risks.map((r) => ({ description: r.description, ...(r.mitigation ? { mitigation: r.mitigation } : {}) })),
        unresolvedQuestions: input.unresolvedQuestions.slice(),
        verificationRequirements: { evidenceRequired: input.evidenceRequired.slice() },
      },
    });
  }

  private mapDependencyType(type: string): HandoffDependencyType {
    if (Object.values(HandoffDependencyType).includes(type as HandoffDependencyType)) return type as HandoffDependencyType;
