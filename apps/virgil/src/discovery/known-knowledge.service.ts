...
import type { CrawlConfig, DiscoveryIntent, IntentCoverage, KnowledgeCoverageResult } from './discovery.schemas.js';
...
export class KnownKnowledgeService {
  ...
  async assess(intent: DiscoveryIntent, config: CrawlConfig): Promise<KnowledgeCoverageResult> {
    ...
    for (const element of intent.elements) {
      ...
      const relevantResults = results.filter((r) => r.score >= config.minRelevanceScore);
      const bestScore = relevantResults.length > 0 ? Math.max(...relevantResults.map((r) => r.score)) : 0;

      let level: CoverageLevel;
      if (relevantResults.length >= FULL_COVERAGE_MIN_MATCHES && bestScore >= config.minRelevanceScore * 2) {
      ...
      coverages.push({ elementKey: element.key, level, bestScore, matchCount: relevantResults.length });
      if (level !== CoverageLevel.FULL) insufficientKeys.push(element.key);
    }
